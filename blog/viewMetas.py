...
def metas_top(request, id, pageID):
    ...
    if not request.user.is_authenticated:
        articles = Articles.objects.order_by('-pub_date').filter(meta_top=id, public=True)
    # 已登录，隐藏除自己外的私密文章
    else:
        articles = (Articles.objects.order_by('-pub_date').filter(
                        Q(meta_top=id, public=True) | Q(meta_top=id, public=False, author=request.user)))
...
def metas_second(request, id, pageID):
    ...
    # 未登录，隐藏私密文章
    if not request.user.is_authenticated:
        articles = Articles.objects.order_by('-pub_date').filter(meta_second=id, public=True)
    # 已登录，隐藏除自己外的私密文章
    else:
        articles = (Articles.objects.order_by('-pub_date').filter(
                        Q(meta_second=id, public=True) | Q(meta_second=id, public=False, author=request.user)))
...
def myArticles(request):
    ...
    secondMetas = SecondMetas.objects.all().order_by('id')
    softMetas = SoftMetas.objects.all().order_by('id')
    articles = Articles.objects.order_by('-pub_date').filter(author=request.user)
